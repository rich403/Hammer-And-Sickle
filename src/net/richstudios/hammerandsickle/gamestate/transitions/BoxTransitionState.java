...
public class BoxTransitionState extends TransitionState {
	...
	public BoxTransitionState(GameStateManager gsm, GameState prev, GameState next) {
		super(gsm, prev, next);
		this.totalTime = 2 * References.FPS;
		rectangles = new ArrayList<Rectangle>();
		widthInterval = Math.round((double) References.WIDTH / (double) this.totalTime);
		heightInterval = Math.round((double) References.HEIGHT / (double) this.totalTime);
	}
	...
	public void update() {
		...
		if (status == EXPANDING) {
			if (timer == 1) {
				rectangles.add(0, new Rectangle(References.WIDTH / 2, References.HEIGHT / 2, 0, 0));
			...
			rectangles.add(new Rectangle(0, 0, References.WIDTH, References.HEIGHT / 2));
			rectangles.add(new Rectangle(0, 0, References.WIDTH / 2, References.HEIGHT));
			rectangles.add(new Rectangle(0, References.HEIGHT / 2, References.WIDTH, References.HEIGHT / 2));
			rectangles.add(new Rectangle(References.WIDTH / 2, 0, References.WIDTH / 2, References.HEIGHT));
			status = COLLAPSING;
